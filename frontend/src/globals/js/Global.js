...
const Global = {
	...
	arr2Str: (num, str) => Array(num).fill(str).join(''),
	...
	emptyStrFromNull: (value, defaultVal = '') => typeof value === 'undefined' || value === null
		? defaultVal : value,

	padWithZero: (num, size = 2) => {
		let s = num + "";
	...
	objectIDsGenerator: (count) => {
		...
		const generatedIds = Array(count).fill().map(_ => objectId());
		return removeDuplicates(generatedIds);
	},
	...
	numberObject: (num) => {
		if (!num) return { num: 0, format: '0.00' };
		return {
			num,
			format: Global.formatNumber(num)
		}
	},
